...
const LoadingSpinner = ({ id, className, disabled = false }) => {
  return (
    <div className={className} style={{ lineHeight: 0, color: 'var(--color-text-0)', '--spinner-width': 'auto' }}>
      <LoadingSpinnerSvg
        id={id}
        width="38"
        height="38"
...
const LoadingSpace = ({ className, description, id }) => (
  <LoadingSpaceContainer className={className}>
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center', alignItems: 'center' }}>
      ...
      {description && <span style={{ marginTop: '1rem' }}>{description}</span>}
    </div>
...
export { LoadingSpinner, LoadingSpace };
